...
# Define the User model - updated without created_at
class User(Base):
    __tablename__ = "users"
...
def save_user_to_db(db, user_id, username, first_name):
    ...
    if not db_user:
        db_user = User(user_id=str(user_id), username=username, first_name=first_name)
        db.add(db_user)
        db.commit()
        db.refresh(db_user)
    return db_user
...
def save_summary_to_db(db, user_id, title, original_text, summary):
    db_summary = Summary(
        user_id=str(user_id),
        title=title,
